 ...
 */
@JobHandler(value="SoccerListJobHandler")
@Component
public class SoccerListJobHandler extends IJobHandler {
	...
	public ReturnT<String> execute(String param) throws Exception {
		...
		Map headResponse = HttpClientUtils.httpPost(RequestUrls.HeadUrl, new Request(headBody));
		...
		Map listResponse = HttpClientUtils.httpPost(RequestUrls.ListUrl, new Request(listBody));
		...
		// 4、开始比较数据
		String[] headListCheckColumns = businessConfig.getHeadList().split(";");
		for(String column : headListCheckColumns) {
			String[] columns = column.split(":");
			if((headMap.get(columns[0]) == null && listMap.get(columns[1]) != null) ||
					(headMap.get(columns[0]) != null && listMap.get(columns[1]) == null) ||
					(headMap.get(columns[0]) != null && listMap.get(columns[1]) != null && !headMap.get(columns[0]).equals(listMap.get(columns[1])))) {
				XxlJobLogger.log("RESULT : 列表与期次数据不一致");
